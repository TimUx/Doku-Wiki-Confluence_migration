...
func run(args []string) error {
	if len(args) > 0 && args[0] == "--version" { fmt.Println(version); return nil }
	...
	cfg, err := config.Load(*path); if err != nil { return err }
	if check { fmt.Println("configuration valid"); return nil }
	db, err := store.Open(cfg.Storage.Database); if err != nil { return err }; defer db.Close()
	...
	go func(){ <-ctx.Done(); shutdown, cancel := context.WithTimeout(context.Background(), 15*time.Second); defer cancel(); _ = httpServer.Shutdown(shutdown) }()
