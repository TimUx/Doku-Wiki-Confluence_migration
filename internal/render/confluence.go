...
func ConfluenceStorage(p model.Page) string {
    ...
    for _, n := range p.Nodes {
        ...
        case "quote": fmt.Fprintf(&b, "<blockquote><p>%s</p></blockquote>\n", storageInline(strings.TrimPrefix(n.Text, ">"), p, attachments))
        case "code": fmt.Fprintf(&b, "<ac:structured-macro ac:name=\"code\"><ac:plain-text-body><![CDATA[%s]]></ac:plain-text-body></ac:structured-macro>\n", strings.ReplaceAll(n.Text, "]]>", "]]]]><![CDATA[>"))
        case "info", "warning", "note": fmt.Fprintf(&b, "<ac:structured-macro ac:name=\"panel\"><ac:parameter ac:name=\"title\">%s</ac:parameter><ac:rich-text-body><p>%s</p></ac:rich-text-body></ac:structured-macro>\n", strings.ToUpper(n.Type), storageInline(n.Text, p, attachments))
...
func storageInline(s string, p model.Page, attachments map[string]string) string {
    ...
    out = confluenceMedia.ReplaceAllStringFunc(out, func(raw string) string {
        ...
        target := resolveTarget(p.ID, m[1]); name := attachments[target]; if name == "" { name = filepath.Base(strings.ReplaceAll(target, ":", "/")) }
