...
func Scan(ctx context.Context, pagesRoot, mediaRoot string) (Result, error) {
	...
	err := filepath.WalkDir(pagesRoot, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			out.Warnings = append(out.Warnings, model.Warning{Code: "read", Message: err.Error()})
			return nil
		}
		select {
		case <-ctx.Done():
		...
		rel, _ := filepath.Rel(pagesRoot, path)
		id := strings.TrimSuffix(filepath.ToSlash(rel), filepath.Ext(rel))
		id = strings.ReplaceAll(id, "/", ":")
		b, e := os.ReadFile(path)
		if e != nil {
			out.Warnings = append(out.Warnings, model.Warning{Code: "read", Message: fmt.Sprintf("cannot read %s", id)})
			return nil
		}
		info, _ := d.Info()
		p := parser.Parse(id, string(b))
		p.SourceFile = path
		p.Namespace = namespace(id)
		p.Size = info.Size()
	...
	err = filepath.WalkDir(mediaRoot, func(path string, d fs.DirEntry, err error) error {
		...
		rel, _ := filepath.Rel(mediaRoot, path)
		info, _ := d.Info()
		out.Media = append(out.Media, model.Media{ID: strings.ReplaceAll(filepath.ToSlash(rel), "/", ":"), Path: path, Size: info.Size(), Modified: info.ModTime()})
		return nil
	})
	return out, err
}
