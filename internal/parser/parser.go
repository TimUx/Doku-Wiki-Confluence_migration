...
func Parse(id, src string) model.Page {
	...
	for i := 0; i < len(lines); i++ {
		...
		for _,m:=range link.FindAllStringSubmatch(raw,-1){target:=unescape(m[1]);kind:="internal";if isExternal(target){kind="external"}else if strings.HasPrefix(strings.TrimSpace(target),"#"){target=strings.TrimSpace(target)}else{target=resolve(id,target)};p.Links=append(p.Links,model.Reference{Target:target,Display:unescape(m[2]),Kind:kind})}
