...
<xyz value="1">`)
	...
	found := false
	for _, n := range p.Nodes { if n.Type == "warning" && n.Meta["kind"] == "warning" { found = true } }
	if !found { t.Fatal("wrap warning not preserved") }
}
...
Snapshots stellen kein Backup dar.
	...
	if bullets != 3 || ordered != 2 { t.Fatalf("lists bullets=%d ordered=%d", bullets, ordered) }
	if rows != 2 { t.Fatalf("table rows=%d", rows) }
	found := false
	for _, n := range p.Nodes { if n.Type == "warning" && n.Meta["kind"] == "important" { found = true } }
	if !found { t.Fatal("important wrap not parsed") }
}
...
func TestExtendedDokuWikiSyntax(t *testing.T) {
	p := Parse("demo:syntax", `====== - Erste Überschrift ======
===== - Unterpunkt =====
...
<WRAP warning>Warnung</WRAP>
	...
	if len(p.Plugins) == 0 { t.Fatal("plugins missing") }
	if p.Plugins[0] != "numberedheadings" { t.Fatalf("plugins=%#v", p.Plugins) }
	if len(p.Includes) != 2 { t.Fatalf("includes=%#v", p.Includes) }
	if p.Includes[0].Kind != "namespace" || p.Includes[1].Kind != "tagtopic" { t.Fatalf("include kinds=%#v", p.Includes) }
	if p.Nodes[0].Type != "heading" || p.Nodes[0].Text != "1 Erste Überschrift" { t.Fatalf("heading=%#v", p.Nodes[0]) }
	foundWrap := false
	for _, n := range p.Nodes {
		if n.Type == "info" || n.Type == "wrap" {
			foundWrap = true
			if n.Meta["kind"] != "info" { t.Fatalf("wrap kind=%#v", n.Meta) }
			if n.Meta["width"] != "50%" { t.Fatalf("wrap width=%#v", n.Meta) }
			if len(n.Children) != 2 { t.Fatalf("wrap children=%#v", n.Children) }
		}
	}
	if !foundWrap { t.Fatal("wrap node missing") }
}

func TestNumberedHeadingExplicitNumber(t *testing.T) {
	p := Parse("demo:numbering", `====== -#5 Kapitel ======
===== - Unterkapitel =====
===== -#8 Sprung =====`)
	if p.Nodes[0].Text != "5 Kapitel" || p.Nodes[1].Text != "5.1 Unterkapitel" || p.Nodes[2].Text != "5.8 Sprung" { t.Fatalf("numbering=%#v", p.Nodes) }
}
